...
public sealed record NemesisConfig
{
    ...
    public bool EnableNemesisNetwork { get; init; } = true;
    public double SpawnChancePercent { get; init; } = 42.0;
    public int MinimumRaidsBetweenAppearances { get; init; } = 0;
    public int MaximumBonusLevels { get; init; } = 16;
    public int LevelGainPerPlayerKill { get; init; } = 2;
...
public sealed record NemesisCompatibilityStatus
{
    ...
    public string? OrbitDetectionPath { get; init; }
    public List<string> OrbitSearchPaths { get; init; } = [];
}

public sealed record NemesisClientConfirmationRequest : IRequestData
{
...
public sealed record NemesisPreparedClientState
{
...
public sealed record NemesisCustomizationIds
{
...
public sealed record NemesisRival
{
    ...
    public string? SourceBotName { get; set; }
    ...
    // v1.0.1 compatibility only. Schema migration reads and clears this value without
    // presenting generated profiles as confirmed live spawns.
    ...
    public NemesisCustomizationIds? CustomizationIds { get; set; }

    // Reads the malformed v1.0.1 "appearance" object during migration. It is never
    // written after migration; valid customization is persisted as explicit IDs.
    [JsonPropertyName("appearance")]
    [JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
    public Customization? LegacyAppearance { get; set; }
...
public sealed class RaidNemesisState
{
    ...
    public bool ProfilePrepared { get; set; }
    public string? RivalId { get; set; }
    public string? PreparedBotId { get; set; }
    public string? PreparedName { get; set; }
    public bool ReturnedFirstInWave { get; set; }
    public bool ClientSelectedProfile { get; set; }
    public bool ClientConfirmedLive { get; set; }
    public bool ClientReportedDead { get; set; }
    public bool ClientReportedAliveUnspawn { get; set; }
    public DateTimeOffset? ClientConfirmedAtUtc { get; set; }
}
